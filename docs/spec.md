...
当前切片在 SQLite 落地前允许使用版本化 JSON store 作为过渡实现，但必须满足与 Zuu Store 一致的可观测和恢复要求：写入必须通过同路径 `.lock` 文件串行化，再采用临时文件加原子替换；读到损坏 JSON 时必须保留 `.corrupt-*.bak` 备份并恢复为空数据；diagnostics 必须暴露每个 store 的路径、记录数、是否存在、是否恢复过、lock 状态以及错误信息，stale lock 必须让对应 store 标记为需要 review。该过渡实现不得把 API Key 或 provider credential 写入这些 JSON 文件。
...
Audit events 查询必须支持 `limit`、`action`、`outcome`、`target`、`authScope`、`authActor`、`authTokenId`、`since` 和 `until` 过滤；`authScope` 只接受 `admin` 或 `read`，`authActor`/`authTokenId` 只能匹配 token 元数据，`since`/`until` 必须是可解析时间并按包含边界过滤 `createdAt`。审计记录不得返回原始 token 或 provider key。
...
- 不允许等待无限期审批，交互式审批必须有超时或可取消边界
- 有在线 UI 时可等待同一个 tool call 的审批结果；无在线用户或调度触发等非交互场景默认拒绝需要交互审批的操作并使 Run 失败
