...
- prompt stream 已携带稳定 `runId`，并可通过 `GET /v1/runs` 和 `GET /v1/runs/:runId` 查询最近运行状态；Agent Run 摘要已使用 `source`、`queued/running/waiting_approval/completed/failed/aborted` 和 `finishedAt`。
...
- `GET/POST/DELETE /v1/schedules`、`GET /v1/schedules/:scheduleId`、`GET /v1/schedules/:scheduleId/runs`、`GET /v1/schedule-runs/:runId`、`POST /v1/schedules/:scheduleId/pause`、`POST /v1/schedules/:scheduleId/resume` 和 `POST /v1/schedules/:scheduleId/trigger` 已支持 Scheduler MVP；当前支持 `once`、`interval`、基础 5 字段 UTC `cron`、prompt action 和 workflow action，并将 schedule run 关联到 Agent Run 或 Workflow Run。Schedule Run 摘要已使用 `scheduledFor`、`startedAt`、`finishedAt` 和 `queued/running/completed/failed/skipped/aborted` 状态，记录持久化到 `.zuu/pi-agent/schedules.json`。
...
- Workflow/subagent package 在当前环境尚未安装，diagnostics 会明确报告缺口；`pi-package` adapter 已有 launch 桥接，但当前 Windows 原生环境会按 `@agwab/pi-workflow` 包页面说明标记为不可用。Scheduler 已有最小内置后端和基础 5 字段 UTC `cron`，但 timezone、misfire、retry、abort schedule run 和真实持久队列仍未落地。
