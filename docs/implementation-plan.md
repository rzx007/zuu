...
- `GET/POST/DELETE /v1/schedules`、`GET /v1/schedules/:scheduleId`、`POST /v1/schedules/:scheduleId/pause`、`POST /v1/schedules/:scheduleId/resume` 和 `POST /v1/schedules/:scheduleId/trigger` 已支持 Scheduler MVP；当前支持 `once`、`interval`、基础 5 字段 UTC `cron`、prompt action 和 workflow action，并将 schedule run 关联到 Agent Run 或 Workflow Run，记录持久化到 `.zuu/pi-agent/schedules.json`。
