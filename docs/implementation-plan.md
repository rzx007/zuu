...
- `src/index.ts`：Hono daemon、浏览器 UI、health、diagnostics、session、run、prompt、abort、compact、new、switch、fork、import API。
...
- `src/client.ts`：轻量 Zuu client，封装 health、diagnostics、sessions、runs、prompt SSE、abort、compact 和 runtime lifecycle 操作。
...
- `pnpm run check` 可以加载应用入口并验证 health/runs/runtime lifecycle client 合同。
...
- WebUI 还没有 session 文件选择器、tree entry 选择器和 JSONL import 表单；对应 lifecycle 能力目前优先面向 client/API。
