...
- `GET/POST/DELETE /v1/schedules`、`GET /v1/schedules/:scheduleId`、`GET /v1/schedules/:scheduleId/runs`、`GET /v1/schedule-runs/:runId`、`POST /v1/schedules/:scheduleId/pause`、`POST /v1/schedules/:scheduleId/resume` 和 `POST /v1/schedules/:scheduleId/trigger` 已支持 Scheduler MVP；当前支持 `once`、`interval`、基础 5 字段 UTC `cron`、prompt action、workflow action 和默认 `overlapPolicy: "skip"`，并将 schedule run 关联到 Agent Run 或 Workflow Run。Schedule Run 摘要已使用 `scheduledFor`、`startedAt`、`finishedAt` 和 `queued/running/completed/failed/skipped/aborted` 状态；同一 schedule 重叠触发会记录为 `skipped`，`reason` 为 `schedule_overlap`；记录持久化到 `.zuu/pi-agent/schedules.json`。
...
- Workflow/subagent package 在当前环境尚未安装，diagnostics 会明确报告缺口；`pi-package` adapter 已有 launch 桥接，但当前 Windows 原生环境会按 `@agwab/pi-workflow` 包页面说明标记为不可用。Scheduler 已有最小内置后端、基础 5 字段 UTC `cron` 和默认 skip overlap，但 timezone、misfire、retry、queue/parallel overlap、abort schedule run 和真实持久队列仍未落地。
