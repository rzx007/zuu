...
- `GET /v1/audit-events` 已支持查询最近审计事件，并可按 `action`、`outcome`、`target` 和 `limit` 过滤；当前会为所有已授权的 `POST/PATCH/DELETE /v1/*` 写操作记录 `api.mutate`，并额外记录 auth rotate、approval resolve 和 package add/trust/install/update/remove 等领域动作，同时避免写入 token/provider key 等密钥。
...
- 当前 API token 仍是单 token 配置，尚未实现权限分级；审计日志已覆盖所有已授权写 API 和常用检索过滤，但尚未记录只读 API 访问。
