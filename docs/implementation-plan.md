...
- Workflow/subagent package 在当前环境尚未安装，diagnostics 会明确报告缺口；`pi-package` adapter 已有 launch 桥接，但当前 Windows 原生环境会按 `@agwab/pi-workflow` 包页面说明标记为不可用。Scheduler 已有最小内置后端、基础 5 字段 cron、IANA timezone、skip/queue/parallel overlap、skip/run_once misfire、有限 retry policy、best-effort schedule run abort 和本地 scheduler lease，能降低多 daemon 误启动导致的重复触发风险；真实生产级 HA 调度后端仍未落地。
