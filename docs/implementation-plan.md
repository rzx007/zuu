...
后续计划应从此切片继续收敛，而不是另起炉灶：Client workspace 包、run registry 持久化、package source/trust/load 管理、package status/install/update/remove operations、approval tool-call 拦截、Vue WebUI approval 操作面、daemon event stream 面板、fake workflow 合约、native workflow backend、native 后台 runner、pi-package launch adapter 和 Scheduler MVP 已经落地。接下来优先增强 WebUI board、task abort 和 production store；`docs/spikes/pi-workflow-runtime.md` 保留为 Linux/WSL2 可选 adapter 验证。
