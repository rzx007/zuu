...
`GET /v1/audit-events` 会返回最近审计事件，并支持 `limit`、`action`、`outcome` 和 `target` 过滤。当前会为所有已授权的 `POST/PATCH/DELETE /v1/*` 写操作记录 `api.mutate`，并额外覆盖 auth rotate、approval resolve 和 package add/trust/install/update/remove 等领域动作；审计记录只写 method/path/status、source、decision、错误摘要等非密钥信息。
