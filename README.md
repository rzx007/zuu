...
Scheduler MVP 已支持 `once`、`interval` 和基础 5 字段 UTC `cron` trigger，支持 prompt action 和 workflow action，记录最近 schedule runs，并可在 WebUI 中创建、暂停、恢复、手动触发和删除。Schedule Run 摘要使用 `scheduledFor`、`startedAt`、`finishedAt` 和 `completed/failed/aborted/skipped` 等状态。timezone、misfire policy、retry policy、abort schedule run 和真实持久队列仍是后续工作。
