...
`subscribeEvents()` 会先按 `afterEventId` replay 已存档事件，再持续接收 daemon live 事件。它默认保存最后事件 ID，断线后用 `Last-Event-ID` 自动重连，并去重重复事件。事件必须包含稳定 `id` 和 `createdAt`；未知事件类型会被忽略，未知字段会保留前向兼容。
